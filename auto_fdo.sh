...
function create_new_wrapper() {
  ...
  echo "${compiler_path}/bin/${c_compiler} -fauto-profile=${profile_data_path}/${gcov_name} \"\$@\"" >${compiler_wrapper}/${c_compiler}
  echo "${compiler_path}/bin/${cxx_compiler} -fauto-profile=${profile_data_path}/${gcov_name} \"\$@\"" >${compiler_wrapper}/${cxx_compiler}
}
